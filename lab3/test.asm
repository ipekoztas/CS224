...
# Linked list is pointed by $v0
	...
	move	$a0, $t0	# Pass the linked list address to $a0
	jal 	printLinkedList
	
	# $a0 value is restored from the stack in printLinkedList
	
	move	$t0, $a0	# Pass the linked list address to $t0
...
# New list is pointed by $v0
	move $a0, $v0		# Pass the newly created linked list address in $a0
	jal printLinkedList
...
# No. of nodes created compared with the number of nodes to be created.
	...
	li	$v0, 9
	syscall
# add the next-address to the first one
	sw	$v0, 0($s2)
...
recursive:
	...
	sw 	$ra, 28($sp)
        ...
        move 	$s4, $a0 		# $s4 original head node address
        lw 	$s6, 0($a0) 		# s6 is the next address
        lw 	$s7, 4($a0) 		# s7 is the value
    	...
    	#Base Case
        beq 	$s6, 0, duplicatedAll
        addi 	$s1, $s0, 8
        sw 	$s1, 0($s0)
        addi 	$s5, $s5, 1 	# increment node counter
        addi 	$a0, $a0, 8 	# link to the next node
    	jal recursive
